...
class Flow:
    ...
    def get_data(self) -> dict:
        ...
        data = {
            ...
            "flow_duration": 1e6 * packet_time.get_duration(), #aaaaaaa
            "tot_fwd_pkts": packet_count.get_total(PacketDirection.FORWARD), #tot_fwd_pkts
            ...
            "fwd_pkt_len_max": float(packet_length.get_max(PacketDirection.FORWARD)), #aaaaaa
            "fwd_pkt_len_min": float(packet_length.get_min(PacketDirection.FORWARD)), #aaaaaa
            # "fwd_pkt_len_mean": float(packet_length.get_mean(PacketDirection.FORWARD)),
            "fwd_pkt_len_std": float(packet_length.get_std(PacketDirection.FORWARD)), #fwd_pkt_len_std
            ...
            "flow_iat_mean": float(flow_iat["mean"]), #flow_iat_mean
            # "flow_iat_std": float(flow_iat["std"]),
            "flow_iat_max": float(flow_iat["max"]), #flow_iat_max
            # "flow_iat_min": float(flow_iat["min"]),
            # "fwd_iat_tot": forward_iat["total"],
            "fwd_iat_mean": float(forward_iat["mean"]), #fwd_iat_mean
            # "fwd_iat_std": float(forward_iat["std"]),
            "fwd_iat_max": float(forward_iat["max"]), #fwd_iat_max
            ...
            "fwd_header_len": flow_bytes.get_forward_header_bytes(), #fwd_header_len
            # "bwd_header_len": flow_bytes.get_reverse_header_bytes(),
            "fwd_pkts_s": packet_count.get_rate(PacketDirection.FORWARD), #fwd_pkts_s
            # "bwd_pkts_s": packet_count.get_rate(PacketDirection.REVERSE),
            "pkt_len_min": packet_length.get_min(), #pkt_len_min
            "pkt_len_max": packet_length.get_max(), #pkt_len_max
            # "pkt_len_mean": float(packet_length.get_mean()),
            "pkt_len_std": float(packet_length.get_std()), #pkt_len_std
            # "pkt_len_var": float(packet_length.get_var()),
            # "down_up_ratio": packet_count.get_down_up_ratio(),
            "pkt_size_avg": packet_length.get_avg(), #pkt_size_avg
            "init_fwd_win_byts": self.init_window_size[PacketDirection.FORWARD], #init_fwd_win_byts
            # "init_bwd_win_byts": self.init_window_size[PacketDirection.REVERSE],
            "fwd_seg_size_min": flow_bytes.get_min_forward_header_bytes(), #fwd_seg_size_min
            ...
            "ack_flag_cnt": flag_count.has_flag("ACK"), #ack_flag_cnt
        ...
        data["subflow_fwd_pkts"] = data["tot_fwd_pkts"] #subflow_fwd_pkts
    ...
    def update_active_idle(self, current_time):
        ...
        if (current_time - self.last_active) > constants.ACTIVE_TIMEOUT:
            ...
            if duration > 0:
                self.active.append(1000000 * duration)
